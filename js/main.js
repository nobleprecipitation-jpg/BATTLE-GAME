...
const savedProgress = JSON.parse(
  localStorage.getItem("quizmon-progress") ||
...
function loadSettings() {
  Object.assign(
    settings,
    JSON.parse(localStorage.getItem("quizmon-settings") || "{}"),
  );
  document.body.classList.toggle("large-text", settings.largeText);
  document.body.classList.toggle("reduced-motion", settings.reducedMotion);
}
function saveSettings() {
  localStorage.setItem("quizmon-settings", JSON.stringify(settings));
}
...
function beep(frequency, duration = 0.08, type = "sine") {
...
function persistProgress() {
  localStorage.setItem("quizmon-progress", JSON.stringify(savedProgress));
}
function unlock(id) {
  if (!savedProgress.unlocked.includes(id)) {
    savedProgress.unlocked.push(id);
    persistProgress();
  }
}
function updateAchievements() {
  ...
  if (getAccuracy() === 100) {
    unlock("perfect-battle");
    unlock("quiz-master");
  }
  if (state.damageTaken === 0) unlock("no-damage");
  if (state.highestCombo >= 5) unlock("combo-five");
  savedProgress.stats.battlesWon = (savedProgress.stats.battlesWon || 0) + 1;
  persistProgress();
}
...
async function loadQuestions() {
  ...
  if (responses.some((response) => !response.ok))
    throw new Error("Lesson data unavailable");
  const [questions, enemyData, achievementData, moveData] = await Promise.all(
    responses.map((response) => response.json()),
  );
  state.enemies = enemyData.enemies.map((enemy, index) => ({
    ...enemy,
    questions: questions.enemies[index].questions,
  }));
  state.achievements = achievementData.achievements;
  Object.assign(moves, moveData.moves);
...
function showDialogue(message, next) {
  ...
  answers.innerHTML = "";
  battleStatus.textContent = "Preparing lesson challenge...";
  typeText(message, () => {
    window.setTimeout(() => {
  ...
  const handleDialogueKey = (event) => {
    if (event.key === "Enter") {
      ...
      if (state.typingActive) {
        clearTimeout(state.typeTimer);
        questionText.textContent = state.currentMessage;
        state.typingActive = false;
        state.locked = false;
        next();
      } else {
...
function typeText(message, onComplete) {
  clearTimeout(state.typeTimer);
  state.currentMessage = message;
  ...
  let index = 0;
  const write = () => {
    questionText.textContent = message.slice(0, index);
    if (index < message.length) {
      index += 1;
      state.typeTimer = window.setTimeout(write, settings.textSpeed);
    } else {
      state.typingActive = false;
...
function getCurrentQuestion() {
  ...
  if (!enemy?.questions) return null;
  return enemy.questions[state.questionIndex] || null;
}
...
function renderQuestion() {
  ...
  battleStatus.textContent = `Question ${state.questionIndex + 1} of 3 · Choose the answer that keeps your connection alive.`;
  answers.innerHTML = "";

  // Randomize choices order dynamically
  const indexedChoices = question.choices.map((choice, origIndex) => ({
    choice,
    isCorrect: origIndex === question.correct,
  }));
  ...
  indexedChoices.forEach((item, index) => {
    ...
    button.className = "answer-button fade-in";
    button.style.animationDelay = `${index * 70}ms`;
    const letter = String.fromCharCode(65 + index);
    button.innerHTML = `<span class="choice-key">${letter}</span><span class="choice-label">${item.choice}</span>`;
    button.addEventListener("click", () => {
      if (state.locked) return;
      state.locked = true;
      initAudio();
      beep(440);
      if (item.isCorrect) correctAnswer(button);
...
function answer(index, button) {
  ...
  const question = getCurrentQuestion();
  if (!question) return;
  if (index === question.correct) correctAnswer(button);
  else wrongAnswer(button);
}
...
function correctAnswer(button) {
  ...
  state.damageDealt += 34;
  button.classList.add("correct");
  enemySprite.classList.add("is-hit", "flash");
  playerSprite.classList.add("is-attacking");
  spawnPlayerProjectile();
  beep(660, 0.16, "triangle");
  showDamage(34, "enemy");
  ...
  const praise =
    enemy.shortName === "ARC"
      ? "Direct hit! You threw your data packet!"
      : enemy.shortName === "JAY"
        ? "Super effective! Jay’s glitch code cracked."
        : "Mastery hit! Boss Pat’s dark matter shatters.";
  ...
  window.setTimeout(() => {
    ...
    enemySprite.classList.remove("is-hit", "flash");
    finishTurn();
  }, 950);
}
...
function wrongAnswer(button) {
  ...
  playerSprite.classList.add("is-hit", "flash");
  enemySprite.classList.add("is-attacking");
  const enemy = state.enemies[state.enemyIndex];
  spawnEnemyProjectile(enemy.shortName);
  const damage = 34;
  state.damageTaken += state.invincible ? 0 : damage;
  beep(180, 0.16, "sawtooth");
  showDamage(state.invincible ? 0 : damage, "player");
  state.playerHealth = Math.max(
    0,
    state.playerHealth - (state.invincible ? 0 : damage),
  );
  updateBars();
  const taunt =
    enemy.shortName === "ARC"
      ? "Arc throws a Pokeball at you! -34 HP"
      : enemy.shortName === "JAY"
        ? "Jay blasts you with a Glitched Hollow Purple! -34 HP"
        : "Boss Pat strikes you with Dark Matter! -34 HP";
  battleStatus.textContent = taunt;
  arena.classList.add("is-shaking");
  window.setTimeout(() => {
    playerSprite.classList.remove("is-hit", "flash");
    enemySprite.classList.remove("is-attacking");
...
function finishTurn() {
  if (state.playerHealth <= 0) {
    return showGameOver(
      "SIGNAL LOST (0 HP)",
      "You made 3 wrong choices and ran out of health! Restart the battle to try again.",
    );
  }
  const enemy = getCurrentEnemy();
  ...
  if (isLastQuestion) {
    if (state.roundCorrect === 3) {
      ...
      return showGameOver(
        "MATCH LOST",
        `You got ${state.roundCorrect}/3 correct. You need at least 2/3 correct answers to beat ${enemy.shortName}!`,
      );
    }
  }
...
function updateNetworkStatus() {
  ...
  if (hp <= 50 && hp > 20) {
    label = "Weak Signal";
    cssClass = "status-weak";
  } else if (hp <= 20 && hp > 0) {
    label = "Critical Connection";
...
function showGameOver(customTitle, customMessage) {
  ...
  const message =
    customMessage ||
    "Your health dropped to zero or you missed too many questions. Restart the battle and try again!";
  showOverlay(
    title,
    message,
    "Retry Battle",
...
function showVictory() {
  ...
  showOverlay(
    "MISSION COMPLETE",
    `You mastered the lesson with ${state.correct} correct answers and ${state.wrong} misses. Accuracy: ${getAccuracy()}%.`,
    "Play Again",
    startGame,
    "View Results",
    showResults,
...
function showResults() {
  ...
  overlayPanel.innerHTML = `<p class="eyebrow">BATTLE REPORT</p><h2>Knowledge unlocked.</h2><div class="stats-grid"><span>ACCURACY<b>${accuracy}%</b></span><span>RANK<b>${getRank(accuracy)}</b></span><span>COMBO<b>${state.highestCombo}x</b></span><span>TIME<b>${elapsed}s</b></span><span>DAMAGE DEALT<b>${state.damageDealt}</b></span><span>DAMAGE TAKEN<b>${state.damageTaken}</b></span></div><button data-overlay-action="close">Return to Title</button>`;
  overlayPanel.querySelector("button").onclick = () => {
    closeOverlay();
    showScreen("title");
  };
}
function showOverlay(
  ...
  overlayPanel.innerHTML = `<h2>${title}</h2><p>${message}</p><button data-overlay-action="primary">${primaryText}</button><button class="secondary" data-overlay-action="secondary">${secondaryText}</button>`;
  overlayPanel.querySelector('[data-overlay-action="primary"]').onclick =
    primaryAction;
  overlayPanel.querySelector('[data-overlay-action="secondary"]').onclick =
    secondaryAction;
}
...
function updateBars() {
  enemyHp.style.width = `${state.enemyHealth}%`;
  playerHp.style.width = `${state.playerHealth}%`;
  enemyHpText.textContent = state.enemyHealth;
  playerHpText.textContent = state.playerHealth;
  comboLabel.textContent = state.combo;
  updateNetworkStatus();
  [enemyHp, playerHp].forEach((bar) => {
    bar.classList.toggle("warning", parseInt(bar.style.width) <= 50);
    bar.classList.toggle("danger", parseInt(bar.style.width) <= 25);
  });
}
function showDamage(amount, target) {
  ...
  number.style.left = target === "enemy" ? "75%" : "25%";
  number.style.top = target === "enemy" ? "35%" : "64%";
  damageLayer.appendChild(number);
  window.setTimeout(() => number.remove(), 1200);
}

function showSettings() {
  overlay.hidden = false;
  overlayPanel.innerHTML = `<h2>SETTINGS</h2><p>Personalize your classroom run.</p><label class="setting-row">Text speed <input id="text-speed" type="range" min="5" max="60" value="${settings.textSpeed}"></label><label class="setting-row"><input id="large-text" type="checkbox" ${settings.largeText ? "checked" : ""}> Large text</label><label class="setting-row"><input id="reduced-motion" type="checkbox" ${settings.reducedMotion ? "checked" : ""}> Reduced motion</label><label class="setting-row"><input id="screen-shake" type="checkbox" ${settings.screenShake ? "checked" : ""}> Screen shake</label><button data-overlay-action="save">Save settings</button>`;
  overlayPanel.querySelector('[data-overlay-action="save"]').onclick = () => {
    settings.textSpeed = Number(
      overlayPanel.querySelector("#text-speed").value,
    );
    settings.largeText = overlayPanel.querySelector("#large-text").checked;
    settings.reducedMotion =
      overlayPanel.querySelector("#reduced-motion").checked;
    settings.screenShake = overlayPanel.querySelector("#screen-shake").checked;
    saveSettings();
    closeOverlay();
  };
}
...
function showTrophies() {
  const cards = state.achievements
    .map((achievement) => {
      const unlocked = savedProgress.unlocked.includes(achievement.id);
      return `<div class="trophy ${unlocked ? "unlocked" : ""}"><b>${unlocked ? achievement.icon : "·"}</b><span>${achievement.name}<small>${unlocked ? achievement.description : "Locked achievement"}</small></span></div>`;
    })
    .join("");
  showOverlay(
    "TROPHY ROOM",
...
function showGallery() {
  showOverlay(
    "GALLERY",
    "Character artwork and stage studies unlock as you complete the Learning League.",
    "Return",
    closeOverlay,
    "Trophy Room",
    showTrophies,
...
function showInstructorMenu() {
  overlay.hidden = false;
  overlayPanel.className = "overlay-panel instructor-panel";
  overlayPanel.innerHTML = `<p class="eyebrow">PRESENTATION MODE</p><h2>Instructor tools</h2><button data-demo="skip">Skip to next rival</button><button data-demo="credits">Jump to results</button><button data-demo="invincible">Toggle invincibility: ${state.invincible ? "ON" : "OFF"}</button><button class="secondary" data-demo="close">Close</button>`;
  overlayPanel.querySelector('[data-demo="skip"]').onclick = () => {
    closeOverlay();
    if (state.enemyIndex < 2) {
      state.enemyIndex += 1;
      loadEnemy();
    }
  };
  overlayPanel.querySelector('[data-demo="credits"]').onclick = showResults;
  overlayPanel.querySelector('[data-demo="invincible"]').onclick = () => {
    state.invincible = !state.invincible;
    showInstructorMenu();
  };
  overlayPanel.querySelector('[data-demo="close"]').onclick = closeOverlay;
}
async function handleAction(action) {
  ...
  if (action === "trophies") showTrophies();
  if (action === "pause" && !battleScreen.hidden)
    showOverlay(
      "BATTLE PAUSED",
      "Take a breath. Your battle state is safe.",
      "Resume",
      closeOverlay,
      "Restart Battle",
      startGame,
...
document.addEventListener("keydown", (event) => {
  if (event.key === "Enter" && !battleScreen.hidden && !overlay.hidden) return;
  if (event.key === "Enter" && !battleScreen.hidden && !state.locked) {
    const first = answers.querySelector("button");
    if (first) first.focus();
  }
  if (event.key === "Escape" && !battleScreen.hidden) handleAction("pause");
});
document.addEventListener("keydown", (event) => {
  if (event.key.toLowerCase() === "p" && !battleScreen.hidden) {
